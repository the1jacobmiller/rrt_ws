...
//apollo
apollo::perception::lidar::MlfEngine mlf_engine;
...
{
  //builder_.Init();
  bool success = mlf_engine.Init();
  if (!success) {
    ROS_ERROR_STREAM("FAiled to Initialize tracker -- MlfEngine");
    exit(0);
  }
...
void CNNSegmentation::run()
{
  ...
  lidar_obstacles_pub = nh_.advertise<visualization_msgs::MarkerArray>("/lidar_obstacles", 1);

  ROS_INFO("[%s] Ready. Waiting for data...", __APP_NAME__);
}

void CNNSegmentation::copyObjectsToApolloLidarFrame(const autoware_msgs::DetectedObjectArray objects,
  apollo::perception::lidar::LidarFrame* frame){

  for (int i = 0; i < objects.objects.size(); i++){
    autoware_msgs::DetectedObject obj = objects.objects[i];
    std::shared_ptr<apollo::perception::base::Object> apolloObject = std::make_shared<apollo::perception::base::Object>();
    apolloObject->id = obj.id;
    apolloObject->confidence = obj.score;
    apolloObject->center = Eigen::Vector3d(obj.pose.position.x, obj.pose.position.y, obj.pose.position.z);
    apolloObject->size = Eigen::Vector3f((float)obj.dimensions.x, (float)obj.dimensions.y, (float)obj.dimensions.z);
    apolloObject->lidar_supplement.is_background = false;
    // geometry_msgs::Quaternion r = obj.pose.orientation;
    // ROS_INFO_STREAM("Yaw: " << obj.pose.orientation.z);
    //
    // double roll, pitch, yaw;
    // tf::Matrix3x3(tf::Quaternion(r.x,r.y,r.z,r.w)).getRPY(roll, pitch, yaw);
    // apolloObject->theta = yaw;
    // ROS_INFO_STREAM("Yaw: " << yaw);
    frame->segmented_objects.push_back(apolloObject);
  }

}

void CNNSegmentation::publishBoundingBoxes(std::vector<std::shared_ptr<apollo::perception::base::Object>> tracked_objects) {
  visualization_msgs::MarkerArray all_objects;
    for(int i = 0; i < tracked_objects.size(); i++) {
      auto obstacle = tracked_objects[i];

      visualization_msgs::Marker text_marker;
      text_marker.header.frame_id = "/os1_lidar";
      text_marker.header.stamp = ros::Time::now();
      text_marker.type = visualization_msgs::Marker::TEXT_VIEW_FACING;
      text_marker.text = std::to_string(obstacle->id);
      text_marker.id = obstacle->id;

      text_marker.scale.x = 1.0;//0.3;
      text_marker.scale.y = 1.0;//0.3;
      text_marker.scale.z = 1.0;//0.1;
      ...
      visualization_msgs::Marker obstacle_cube;
      obstacle_cube.header.frame_id = "/os1_lidar";
      obstacle_cube.header.stamp = ros::Time::now();
      obstacle_cube.action = visualization_msgs::Marker::ADD;
      obstacle_cube.id = 10000 + obstacle->id;
      obstacle_cube.lifetime = ros::Duration(0.1);
      obstacle_cube.type = visualization_msgs::Marker::CUBE;
      // define scale
      obstacle_cube.scale.x = obstacle->size[0];
      obstacle_cube.scale.y = obstacle->size[1];
      obstacle_cube.scale.z = obstacle->size[2];
      // define color
      obstacle_cube.color.r = 0.0;
      obstacle_cube.color.b = 1.0;
      obstacle_cube.color.g = 0.0;
      obstacle_cube.color.a = 0.5;

      geometry_msgs::Point p;
      p.x = obstacle->center[0]; //(obstacle.center.x * cos(localizer_yaw) - obstacle.center.y * sin(localizer_yaw)) + localizer_position.lon;
      p.y = obstacle->center[1]; //(obstacle.center.x * sin(localizer_yaw) + obstacle.center.y * cos(localizer_yaw)) + localizer_position.lat;
      p.z = obstacle->center[2];
      obstacle_cube.pose.position = p;
      obstacle_cube.pose.orientation = toQuaternion(0.0, 0.0, obstacle->theta);
      text_marker.pose.position = p;
      text_marker.pose.position.z = 3.0;
      all_objects.markers.push_back(obstacle_cube);
      all_objects.markers.push_back(text_marker);

    }
    printf("Publishing lidar objects\n");
    lidar_obstacles_pub.publish(all_objects);
...
void CNNSegmentation::pointsCallback(const sensor_msgs::PointCloud2 &msg)
{
  ...
  transform2.translation() << 0.0, 0.0, 0.0;
  transform2.rotate(Eigen::AngleAxisf((0.0*M_PI) / 180, Eigen::Vector3f::UnitX()));
  transform2.rotate(Eigen::AngleAxisf((5.5*M_PI) / 180, Eigen::Vector3f::UnitY()));
  transform2.rotate(Eigen::AngleAxisf((0.0*M_PI) / 180, Eigen::Vector3f::UnitZ()));
  pcl::transformPointCloud(*in_pc_ptr, *in_pc_ptr, transform2); //Only rotate target cloud
  ...
  //ROS_INFO_STREAM("detected objects size: " << objects.objects.size());
  ...
  frame.sensor_info.type = apollo::perception::base::SensorType::VELODYNE_64;
  frame.sensor_info.frame_id = "velodyne64";
  frame.timestamp = msg.header.stamp.toNSec();

  copyObjectsToApolloLidarFrame(objects, &frame);

  //mlf_engine.Track(&frame);

  publishBoundingBoxes(frame.segmented_objects);
  ROS_INFO_STREAM("segmented objects size: " << frame.segmented_objects.size());
