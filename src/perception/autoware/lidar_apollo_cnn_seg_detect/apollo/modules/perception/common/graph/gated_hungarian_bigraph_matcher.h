...
void GatedHungarianMatcher<T>::Match(
    ...
    std::vector<size_t>* unassigned_rows,
    std::vector<size_t>* unassigned_cols) {
  check_notnull_mod(assignments);
  check_notnull_mod(unassigned_rows);
  check_notnull_mod(unassigned_cols);
  ...
  check_eq_mod(row_components.size(), col_components.size());
...
void GatedHungarianMatcher<T>::ComputeConnectedComponents(
    std::vector<std::vector<size_t>>* row_components,
    std::vector<std::vector<size_t>>* col_components) const {
  check_notnull_mod(row_components);
  check_notnull_mod(col_components);
