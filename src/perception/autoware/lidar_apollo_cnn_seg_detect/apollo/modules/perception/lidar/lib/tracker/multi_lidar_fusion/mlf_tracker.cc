...
bool MlfTracker::Init(const MlfTrackerInitOptions options) {
  ...
  //done-todoConf
  //std::string filters[2] = {"MlfShapeFilter", "MlfMotionFilter"};

  //for (int i = 0; i < 2; ++i) {
    //const auto& name = filters[i];
    MlfBaseFilter* filter = new apollo::perception::lidar::MlfShapeFilter;//MlfBaseFilterRegisterer::GetInstanceByName(name);
    check_mod(filter);
    MlfFilterInitOptions filter_init_options;
    check_mod(filter->Init(filter_init_options));
    filters_.push_back(filter);
    std::cout << "MlfTracker add filter: " << filter->Name();

    //2nd
    MlfBaseFilter* filter2 = new apollo::perception::lidar::MlfMotionFilter;//MlfBaseFilterRegisterer::GetInstanceByName(name);
    check_mod(filter2);
    MlfFilterInitOptions filter_init_options2;
    check_mod(filter2->Init(filter_init_options2));
    filters_.push_back(filter2);
    std::cout << "MlfTracker add filter: " << filter2->Name();
  //}
