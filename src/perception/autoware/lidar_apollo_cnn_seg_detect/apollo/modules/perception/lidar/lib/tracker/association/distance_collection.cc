...
float LocationDistance(
    const TrackedObjectConstPtr& last_object,
...
float DirectionDistance(
  ...
  Eigen::Vector3f new_anchor_point =
    (new_object->anchor_point).cast<float>();
...
float BboxIouDistance(
  ...
  if (change_cur_obj_bbox) {
    new_dir = old_dir;
    common::CalculateBBoxSizeCenter2DXY(cloud,
        new_dir,
        &new_size,
        &new_center,
        minimum_edge_length);
  ...
  double iou = common::CalculateIou2DXY<double>(old_center,
      old_size_tmp,
      new_center,
      new_size_tmp);
  // Step 4: compute dist
