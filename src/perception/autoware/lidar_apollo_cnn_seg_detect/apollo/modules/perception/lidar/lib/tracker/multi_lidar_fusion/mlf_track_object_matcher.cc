...
//   if (!b) { std::cout<<"CHECK FAILED!!!"<<std::endl; exit(0); }
// }
...
bool MlfTrackObjectMatcher::Init(
  ...
  //apollo::perception::lidar::MultiHmBipartiteGraphMatcher testMatcher;
  //apollo::perception::lidar::GnnBipartiteGraphMatcher testMatcher2;


  foreground_matcher_.reset(new apollo::perception::lidar::MultiHmBipartiteGraphMatcher);  //done-todoConf
  check_mod(foreground_matcher_ != nullptr);
  std::cout << "MlfTrackObjectMatcher, fg: " << foreground_matcher_->Name();
  background_matcher_.reset(new apollo::perception::lidar::GnnBipartiteGraphMatcher); //done-todoConf
  check_mod(background_matcher_ != nullptr);
  std::cout << "MlfTrackObjectMatcher, bg: " << background_matcher_->Name();
  ...
  check_mod(track_object_distance_->Init(distance_init_options));

  //done-todoConf
  //bound_value_ = config.bound_value();
  bound_value_ = 100;
  //max_match_distance_ = config.max_match_distance();
  max_match_distance_ = 4.0;
  return true;
}
