...
bool MlfTrackObjectDistance::Init(
  ...
  //done-Conf
  // for (int i = 0; i < config.foreground_weights_size(); ++i) { //done-Conf
  //   const auto& fgws = config.foreground_weights(i); //done-Conf
  ...
  // }
  // for (int i = 0; i < config.background_weights_size(); ++i) { //done-Conf
  //   const auto& bgws = config.background_weights(i); //done-Conf
...
float MlfTrackObjectDistance::ComputeDistance(
  ...
  check_mod(weights != nullptr && weights->size() >= 7);
  float distance = 0.f;
  float delta = 1e-10f;
